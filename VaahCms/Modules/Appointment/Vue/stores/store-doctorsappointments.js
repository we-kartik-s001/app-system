...
let model_namespace = 'VaahCms\\Modules\\Appointment\\Models\\DoctorsAppointment';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/appointment/doctorsappointments";
...
export const useDoctorsAppointmentStore = defineStore({
    id: 'doctorsappointments',
    state: () => ({
        base_url: base_url,
        ajax_url: ajax_url,
        ...
        route: null,
        watch_stopper: null,
        route_prefix: 'doctorsappointments.',
        view: 'large',
        show_filters: false,
        list_view_width: 12,
        form: {
    ...
    actions: {
        ...
        setViewAndWidth(route_name)
        {
            switch(route_name)
            {
                case 'doctorsappointments.index':
                    this.view = 'large';
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'doctorsappointments.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'doctorsappointments.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'doctorsappointments.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'doctorsappointments.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'doctorsappointments.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
...
// Pinia hot reload
if (import.meta.hot) {
    import.meta.hot.accept(acceptHMRUpdate(useDoctorsAppointmentStore, import.meta.hot))
}
